...
| (d) **Current bottleneck** | — | **L2/L3 are stubbed.** When PubMed name disambiguation returns >1 strong match, there is no ORCID fallback to break the tie, so ambiguous names stay low-confidence **forever** — the loop can't improve them on any future run. The bottleneck is not speed; it's the missing disambiguation stage. Second-order: serial per-person PubMed calls (fine at ~15 people). |
