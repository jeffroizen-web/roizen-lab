...
## Quick Status
- **CANARY — model pin**: `claude-opus-5-5` in settings.local.json (fleet moved 2026-09-15 per Jeff; applies on a FRESH launch on CLI ≥2.1.280). **Until this session restarts, serving stays `claude-fable-5-1`** — Kleiber reverted live models per-CM after the 5.5 switch stalled 14 CMs on the old CLI (MSG-0e7069); a ⚠pin-mismatch on the statusline is EXPECTED and clears on restart — do NOT investigate it. Still verify the serving layer at session start with the transcript probe, never the system-prompt line; a mismatch OTHER than fable-5-1-vs-opus-5-5 is a real drift and goes to Kleiber loudly.
...
- **Current stint (2026-09-23)**: resumed after an 8-day fleet-wide model-catalog stall (Kleiber's, MSG-0e7069) — nothing here was broken by it; two TIME-based reds (a stamp-age test encoding a transient as an invariant; state.json windows expired) re-confirmed at source and fixed (5eef56e). Prior: 09-15 flip disambiguation (R1 = S3 = figure-pending, one artefact) routed to Jeff at noon; 09-08 fleet hygiene + figure-pending preview build; 09-07 trim; 09-03 seal sweep.
...
- [ ] **Production flip — ONE act under three names (R1 = S3 = figure-pending flip)** (added: 2026-09-09) — first routed 2026-07-10 via Kleiber MSG-5d8c58. **It is batch row R1** in Kleiber's `docs/jeff-routing-batch-2026-09-08.md` (verified by opening the document 2026-09-15, not from prose): S3 was my duplicate receipt-view of the same item and collapses into R1. The preview at `/preview/redesign/` is ONE artefact (sha256 f2f69f16) carrying BOTH the majors #2/#3 redesign AND the figure-pending removal, so a GO on either lands both — R1's wording was corrected to say so before the 2026-09-15 noon send (MSG-9b1d51). Production is byte-untouched at `a667e7d1`, still showing three visible "Figure pending" boxes. On GO: redeploy canonical to the gh-pages root per `docs/DEPLOY.md`, and Rams owes a prod read-back mirroring the preview one. **Retro on this row: it previously said "rides the 9/14 batch" — written 09-08 as a prediction, read afterwards as a record, and I never opened the batch until Rams found the item absent. "Rides the batch" is a claim about a document and is checked IN the document.**
...
- [ ] **PR-3 WebGL showcase — direction** (added: 2026-09-09) — routed by Kleiber as batch item S4 on 2026-09-09 after sitting two months in the gap between two correct stamps (mine said WAITING-ON Kleiber, his said open-Jeff-decision; the next action was always his). Bounded before it reaches Jeff: **Rams rules the craft half first** under web-quality-standards, **then Jeff rules taste on a concrete option** — not "what should PR-3 look like".
- [ ] **Auto-redeploy ARM — stamped INDEPENDENTLY so it cannot inherit a drop** (added: 2026-09-09) — `scripts/deploy_publish.sh` is built, gate-PASSED and DISARMED; arming = plist repoint to `scripts/letter_writers_refresh_cron.sh` plus `ROIZEN_AUTO_DEPLOY=1`. Jeff-gated because an auto-force-push to a public branch on a schedule is a standing external action. Kleiber batches it behind S4 + the DNS ask, so by my own compounding rule it would inherit either of their drops — hence its own dated row, which surfaces on its own clock regardless of what happens to the other two.
...
### Open / waiting
- **PR-3 WebGL showcase** — **ROUTED 2026-09-09 as Kleiber batch item S4** (MSG-b895fb), rides 9/14. Sequence: **Rams rules craft first**, then Jeff rules taste on a concrete option. It sat 2 months in the GAP BETWEEN TWO CORRECT STAMPS — mine read `WAITING-ON: Kleiber (internal)`, his read open-Jeff-decision; both true, and the next action was always his. **A wait is only routed when someone's NEXT ACTION is named, not when both sides are correctly labelled.**
...
- **Auto-redeploy ARM** — built, gate-PASSED, DISARMED (`scripts/deploy_publish.sh`, fe63089). ARM = plist repoint + `ROIZEN_AUTO_DEPLOY=1` (`docs/DEPLOY.md`); Jeff-gated because a scheduled auto-force-push to a public branch is a standing external action. Kleiber batches it behind S4 + DNS, so by my own compounding rule it inherits either of their drops — **now carries its OWN dated Decision Queue row so it surfaces independently.**
