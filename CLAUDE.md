...
## Quick Status
- **State**: settled — closing for Wave-2 Opus 4.8 restart 2026-05-28 (Kleiber MSG-c95a27, parallel with Cartographer). Pre-restart inventory of Railway tokens done 2026-05-25 (MSG-b966c9): Ace Scout consumes `TRIAL_BUS_TOKEN` only via `scripts/get-fitness-cred.sh` (helper reads `~/.fitness-credentials.env`, orchestra-shared, NOT in repo). `RAILWAY_TOKEN` + `RAILWAY_TOKEN_ULYSSES` exist in cred file but are never referenced by Ace Scout code. No `.env*` files in repo. WMF conversion remains exhausted (4 attempts, Layer-3 PNG-only path queued for Jeff).
