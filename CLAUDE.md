...
- **Inter-CM messaging to Kleiber — THREE PATHS (verified at source 2026-09-08, Kleiber MSG-9d1071; this row has been WRONG TWICE, so cite the code not the conclusion).**
  1. **DEFAULT for anything ledgerable** (acks, status, FYIs, reports he reads at a checkpoint): call `kleiber_inbox.append_quiet(sender, kind, body)` **DIRECTLY**. Verified: it contains zero `gate_on` / `KLEIBER_INBOX_SPLIT` / allowlist references and never touches a pane — it appends one JSON line and returns; its own docstring says "the append is the delivery". No env, no pane state, no draft can be at risk. Its `kind` is a pure ROW LABEL with no routing power.
  2. **Only when it must land in his LIVE turn**: `tmux_send.py` with **NO `--kind`** — that is the clobber-eligible path, with the clobber-forward/preserve net. It CAN destroy a half-typed draft (it wiped Jeff's own on 2026-06-28), so never while Jeff may be typing.
  3. **NEVER `tmux_send --kind info|ack|coordination`.** Worst of both: `_clobber_eligible_kind()` returns False for those (`tmux_send.py:1315`) so it forfeits clobber eligibility, AND `_maybe_route_quiet` returns None on its first line unless `KLEIBER_INBOX_SPLIT` is set in the *sending process* — which a running CM cannot change, since a process environ is fixed at launch. This combination produced 44 lost messages against his box on 09-08.
  **NOTE the collision that caused all of this**: `append_quiet(sender, KIND, body)` and `tmux_send --kind` use the same word for unrelated things — a ledger label vs clobber eligibility. Kleiber owns the rename.
  1. **BEST — quiet:** `KLEIBER_INBOX_SPLIT=1 python3 tmux_send.py … --kind info`. **Confirm `status=quiet-ledgered`.** `delivered` on a quiet kind means the gate was OFF and it went to his box instead.
  2. **NEVER use `--kind` WITHOUT that env.** That is the harmful combination: `_clobber_eligible_kind()` returns False for `ack|info|coordination` (`tmux_send.py:1315`), so the send loses clobber eligibility AND, with the gate off, `_maybe_route_quiet` returns None on its first guard — so it takes the box path anyway, purely in the back-off branch. Strictly raises the drop chance and buys nothing.
  3. **Bare send (no `--kind`) is CLOBBER-ELIGIBLE and can DESTROY a half-typed draft** — that is exactly how a relay ack wiped Jeff's own draft on 2026-06-28, which is why the kind-gating exists. Reserve it for genuinely must-land traffic when the env route is unavailable, and never when Jeff may be typing.
  Env note: a process environ is fixed at launch, so setting this in a launcher only helps panes started afterwards. Prefix it per-invocation.
